#!/bin/bash

if [ $# -ne 3 ]
then
  echo "Usage: $0 <CLIENT_KEY_NAME>"
  exit
fi

KEY_NAME=$1
HELM_RELEASE=$2
POD_NAME=$(kubectl get pods -l "app=openvpn" -o jsonpath='{.items[0].metadata.name}')
SERVICE_NAME=$(kubectl get svc -l "app=openvpn" -o jsonpath='{.items[0].metadata.name}')
SERVICE_IP=$(kubectl get svc "$SERVICE_NAME" -o go-template='{{range $k, $v := (index .status.loadBalancer.ingress 0)}}{{$v}}{{end}}')
kubectl exec -it "$POD_NAME" /etc/openvpn/setup/newClientCert.sh "$KEY_NAME" "$SERVICE_IP"
kubectl exec -it "$POD_NAME" cat "/etc/openvpn/certs/pki/$KEY_NAME.ovpn" > "$KEY_NAME.ovpn"